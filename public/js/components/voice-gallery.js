...
const GENDER_ICON = { female: "venus", male: "mars" };
...
export class VoiceGallery {
  ...
  render(voices) {
    ...
    if (!voices.length) {
      this.container.innerHTML =
        '<p class="gallery-empty">No voices available.</p>';
      return;
    }
    ...
    voices.forEach((v) => {
      ...
      const genderIconName = GENDER_ICON[v.gender?.toLowerCase()] || "circle";
      ...
      card.innerHTML = `
                ...
                <div class="voice-card-info">
                    ...
                    <div class="voice-card-tags">
                        ${v.gender ? `<span class="voice-tag"><i data-lucide="${genderIconName}" class="icon icon-sm"></i>${escapeHtml(v.gender)}</span>` : ""}
